...
(function(){
  ...
  function createDayCell(day, other, year, month){
    ...
    cell.innerHTML = `<div class="calendarDayNum">${day}</div><div class="calendarEventTitle" id="events-${day}"></div>`;
    return cell;
  }

  function updateCalendarEvents(tasks){
    // Clear month's events
    for(let i = 1; i <= 31; i++){
      const el = document.getElementById(`events-${i}`);
    ...
    tasks.forEach(t => {
      ...
      const day = d.getDate();
      const el = document.getElementById(`events-${day}`);
      if(el){
